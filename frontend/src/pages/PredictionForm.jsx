...
function PredictionForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error || "Prediction failed. Please check your inputs and try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-xl mx-auto px-6 py-10">
      ...
      {/* Page header */}
      <div className="mb-8">
        <h1 className="text-2xl font-semibold text-gray-900">Predict your result</h1>
        <p className="text-sm text-gray-500 mt-1">
          Enter your academic data below to get an instant pass/fail prediction.
        </p>
      </div>

      {/* Show result card or form */}
      {result ? (
            ...
            {fields.map((f) => (
              <div key={f.name}>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {f.label}
                  <span className="text-gray-400 font-normal ml-1">({f.tip})</span>
                </label>
